...
function tryExec(sql) {
  try { db.exec(sql); } catch (err) {
    if (!/duplicate column name|already exists|no such column/i.test(err.message)) throw err;
  }
}

// schema.sql berisi definisi FINAL (multi-akun). Supaya DB LAMA yang struktur
// tabelnya masih versi awal tidak membuat index baru gagal ("no such column"),
// jalankan schema per-statement & lewati error "no such column": kolom-kolom
// itu baru ditambahkan migrasi v2 di bawah, yang juga membuat ulang index-nya.
for (const stmt of fs.readFileSync(SCHEMA_PATH, 'utf8')
...
const MIGRATIONS = [
  ...
  () => {
    ...
    if (/chat_id\s+TEXT\s+UNIQUE/i.test(tblSql)) {
      // Matikan pemeriksaan FK selama rebuild — DB lama punya baris yatim
      // (misal job_target -> target yang sudah dihapus), dan saat DROP TABLE
      // SQLite menyalin struktur FK ke tabel hasil RENAME.
      db.pragma('foreign_keys = OFF');
      db.transaction(() => {
        const cols = db.prepare('PRAGMA table_info(targets)').all().map((c) => c.name);
        // Kolom yang TIDAK ada di tabel lama harus ditambahkan ke CREATE TABLE baru,
        // tapi JANGAN dimasukkan ke daftar INSERT SELECT (karena tidak ada sumbernya).
        const missing = ['account_id', 'bot_token_id', 'can_send_bots'].filter((c) => !cols.includes(c));
        const present = (name) => (cols.includes(name) ? `, ${name} TEXT` : '');
        db.exec(`CREATE TABLE targets_new (
          ...
          ${missing.includes('account_id') ? '' : ', account_id INTEGER'}
          ${missing.includes('bot_token_id') ? '' : ', bot_token_id INTEGER'}
          ${present('can_send_bots')}
        )`);
        // INSERT SELECT eksplisit per-kolom — jumlah & urutan kolom lama bervariasi
        // antar versi DB, jadi jangan pernah pakai `SELECT *`.
        db.exec(`INSERT INTO targets_new (${cols.join(', ')}) SELECT ${cols.map((c) => `"${c}"`).join(', ')} FROM targets`);
        // Pindahkan baris yatim (broadcast_job_targets -> target yang sudah tidak ada)
        // supaya FK bisa diaktifkan lagi setelah rebuild — tabel lama dibuat tanpa
        // pemeriksaan FK sehingga baris menggantung mungkin tertinggal.
        db.exec('DELETE FROM broadcast_job_targets WHERE target_id NOT IN (SELECT id FROM targets)');
...
const stmtListJobDetails = db.prepare(`
  SELECT bjt.id, bjt.target_id, bjt.order_index, bjt.method, bjt.status, bjt.sent_at, bjt.error_msg, bjt.retry_count,
